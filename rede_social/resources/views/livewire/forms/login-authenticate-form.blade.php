<form class="max-w-full mx-10" wire:submit.prevent="authUserLogin">


    <div class="relative z-0 w-full mb-5 group">

        <label class="flex w-full mb-2 items-center justify-between text-md font-medium">
            <p class=' justify-self-start'>E-mail</p>
            @error('email')
                <div class="text-sm text-rose-600 justify-self-end">{{ $message }}</div>
            @enderror
        </label>
        ...
        <div class="relative">
            <input type="text" wire:model="email" maxlength="40"
                class=" 
                bg-indigo-50 border border-indigo-300 text-indigo-900 text-sm font-md rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5  "
    ...
    <div class="relative z-0 w-full mb-5 group">
        ...
        <label class="flex w-full mb-2 items-center justify-between text-md font-medium">
            <p class=' justify-self-start'>Senha</p>
            @error('password')
                <div class="text-sm text-rose-600 justify-self-end">{{ $message }}</div>
            @enderror
        </label>
    ...
    <!-- Or use session flash to handle messages -->
    @isset($returnMessage['status'])
        @if($returnMessage['status'] == 'error')

            <div id="alert-1" class=" relative z-0 w-auto flex-inline bubble-animation">
            ...
            @elseif ($returnMessage['status'] === 'success')
