...
<div class="relative bg-indigo-600 text-slate-50 rounded-lg shadow">
    ...
    <div class="flex items-center justify-between mx-2 sm:mx-4 py-2 sm:py-4 border-b border-slate-50 rounded-t">
        ...
        <button type="button"
            ...
            data-modal-hide="edit-post-modal-{{$postId}}">
    ...
    <div class="p-2 sm:p-4 space-y-4">
        ...
        <form wire:submit.prevent="update" enctype="multipart/form-data">
            ...
            <textarea name="" maxlength="255" wire:model="content"
                ...
                id="" placeholder="Digite aqui o conteúdo do meu post...">{{$content}}</textarea>

            @error('files')
                <div class="text-sm font-medium text-rose-400 justify-self-end me-2">{{ $message }}</div>
            @enderror
            @if ($uploads || $oldUploads)

            @include('components.uploads-media-carousel', ['uploads' => $uploads, 'oldUploads' => $oldUploads])
            
                <div class="mt-1 border-t border-slate-50">
                    <h3>Arquivos anexados:</h3>
                    <ul>
                        @foreach ($oldUploads as $index => $upload)
                            <li class="flex gap-3 mb-1 ps-1.5 bg-indigo-500 rounded-full w-fit ">
                                <p class="truncate overflow-hidden">{{ $upload->file }}</p>
                                <div wire:click="removeOldUploads({{$index}})" class="justify-self-end" role="button"
                        ...
                        @foreach ($uploads as $index => $upload)
                            <li class="flex gap-3 mb-1 ps-1.5 bg-indigo-500 rounded-full w-fit ">
                                <p class="truncate overflow-hidden">{{ $upload->getClientOriginalName() }}</p>
            ...
            <label for="file_zone" class="flex items-end justify-end w-full">
                <div class="absolute me-2 mb-2">
                    <div role="button"
                        ...
                        <svg class="w-6 h-6 slate-50" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
            ...
            <div
                ...
                <button type="button" data-modal-hide="edit-post-modal-{{$postId}}"
